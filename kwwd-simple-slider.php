...
define('KWWD_Slider_DIR',     plugin_dir_path(__FILE__));
define('KWWD_Slider_URL',     plugin_dir_url(__FILE__));
...
// ── Includes ──────────────────────────────────────────────────────────────────
require_once KWWD_Slider_DIR . 'includes/global-settings.php'; // Must come before db.php (KWWDSlider_get_active_slider_settings depends on it)
require_once KWWD_Slider_DIR . 'includes/db.php';
require_once KWWD_Slider_DIR . 'includes/shortcode.php';
require_once KWWD_Slider_DIR . 'includes/admin.php';

// ── Activation ────────────────────────────────────────────────────────────────
register_activation_hook(__FILE__, 'KWWD_Slider_activate');

function KWWD_Slider_activate() {
    KWWD_Slider_create_tables();
}

// ── Scripts & Styles ──────────────────────────────────────────────────────────
add_action('wp_enqueue_scripts', 'KWWD_Slider_frontend_assets');

function KWWD_Slider_frontend_assets() {
    wp_enqueue_style(
        'swiper',
    ...
    wp_enqueue_style(
        'kwwd-slider',
        KWWD_Slider_URL . 'assets/slider.css',
        ['swiper'],
        KWWD_Slider_VERSION
    );
    wp_enqueue_script(
        'kwwd-slider',
        KWWD_Slider_URL . 'assets/slider.js',
        ['swiper'],
        KWWD_Slider_VERSION,
        true
    );
...
add_action('admin_enqueue_scripts', 'KWWD_Slider_admin_assets');

function KWWD_Slider_admin_assets($hook) {
    if (strpos($hook, 'kwwd-slider') === false) return;
    wp_enqueue_media();
    wp_enqueue_style(
        'kwwd-slider-admin',
        KWWD_Slider_URL . 'assets/admin.css',
        [],
        KWWD_Slider_VERSION
    );
    wp_enqueue_script(
        'kwwd-slider-admin',
        KWWD_Slider_URL . 'assets/admin.js',
    ...
    wp_localize_script('kwwd-slider-admin', 'wkrnSlider', [
        ...
        'nonce'   => wp_create_nonce('KWWD_Slider_nonce'),
    ]);
}
?>